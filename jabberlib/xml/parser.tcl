...
proc XML:extract {vars} {
  ...
  while {$tmp==""} {
    if {[set place1 [findfirst "=" $vars 0]]==-1} {
      return $listvar
    }
    ...
    if {$tmp1<$tmp2 && $tmp1!="-1"} {
      set place2 $tmp1
      set quotchar {"}
    } elseif {$tmp1>$tmp2 && $tmp2=="-1"} {
      set place2 $tmp1
      set quotchar {"}
    } elseif {$tmp2<$tmp1 && $tmp2!="-1"} {
      set place2 $tmp2
      set quotchar {'}
    } elseif {$tmp2>$tmp1 && $tmp1=="-1"} {
      set place2 $tmp2
      set quotchar {'}
    } else {
      return $listvar
    ...
    #If there is something like {name=  abcd='aa'} ,skip name parameter
    if {[string trim [string range $vars [expr $place1+1] [expr $place2-1]]]!=""} {
      set vars [string range $vars [expr $place1+1] end]
      continue
    }
