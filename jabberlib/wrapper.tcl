...
proc wrapper:new {streamstartcmd streamendcmd parsecmd} {
  ...
  set wrapper($id,level1tag) ""           ;# Just something, to know that we're under <stream:stream> tag, not <anythingelse>
  set wrapper($id,xmldata) ""             ;# Level 2 xmldata, level1 is the main tag, <stream:stream>, and level2 is the command tag, such as <message>. We don't handle level1 xmldata, because we do not need a list of tags/childtags that we received in a jabber session.
...
proc wrapper:getlevel {id level} {
  ...
  for {set i 2} {$i != $level} {incr i} {
	if {[llength $a] < 5} {return} else {set a [lindex $a end]}
  }
  return $a
}
...
# Check args, to see if empty element
  ...
  if {$wrapper($id,level) == 0} {
	...
	if {$tagname == "stream:stream"} {uplevel #0 "$wrapper($id,streamstartcmd) [list $varlist]"}
...
set isempty 0
  ...
  if {$wrapper($id,level) == 1} {
	if {$tagname != $wrapper($id,level1tag)} {wrapper:xmlerror $id "Open tag \"[lindex $wrapper($id,tempdata) 0]\" doesn't match with close tag \"$tagname\""; return}
	if {$tagname == "stream:stream"} {uplevel #0 $wrapper($id,streamendcmd); return}
  ...
  if {[lindex $wrapper($id,tempdata) 0] != $tagname} {wrapper:xmlerror $id "Open tag \"[lindex $wrapper($id,tempdata) 0]\" doesn't match with close tag \"$tagname\""; return}
  ...
  if {$wrapper($id,level) == 1} {
	if {$wrapper($id,level1tag) == "stream:stream"} {uplevel #0 "$wrapper($id,parsecmd) [list $wrapper($id,tempdata)]"}
	set wrapper($id,tempdata) ""
	set wrapper($id,xmldata) ""
	return
  }
...
proc wrapper:createxml {xmldata} {
  ...
  if {[llength $xmldata] < 5} {set subtags ""} else {set subtags [lrange $xmldata 4 end]}

  set a "<$tagname"; set retext $retext$a
  foreach {attr value} $vars {
	set a " $attr='[wrapper:xmlcrypt $value]'"; set retext $retext$a
  }
  if {$isempty == 1 && $chdata == "" && [llength $subtags] == 0} {
	set a "/>"; set retext $retext$a
	return $retext
  } else {
	set a ">"; set retext $retext$a
  }

  set a [wrapper:xmlcrypt $chdata]; set retext $retext$a

  foreach subdata $subtags {
	set a [wrapper:createxml $subdata]; set retext $retext$a
  }
